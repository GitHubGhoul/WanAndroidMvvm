...
sealed class ResultState<out T> {
    companion object {
        ...
        fun <T> onAppError(error: BaseException): ResultState<T> = Error(error)
    }
    data class Loading(val loadingMessage:String) : ResultState<Nothing>()
    data class Success<out T>(val data: T) : ResultState<T>()
    data class Error(val error: BaseException) : ResultState<Nothing>()
}
...
fun <T> MutableLiveData<ResultState<T>>.paresResult(result: BaseResponse<T>) {
    value = when {
        result.isSuccess() -> {
            ...
            ResultState.onAppError(BaseException(result.getResponseCode(), result.getResponseMsg()))
        }
    }
}
